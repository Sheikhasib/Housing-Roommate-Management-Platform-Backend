...
- **Money**: all amounts `Decimal(10,2)`. Multi-gateway payments (bKash/SSLCommerz/Stripe) run behind `lib/payments/` adapters: adapters own provider HTTP only, `lib/payments/settle.ts` is the single settle path (amount-verified against the initiation snapshot in minor units — mismatch stays PROCESSING + audit). Payment/invoice statuses change **only** via provider-verified notify routes (bKash callback execute / SSLCommerz validator `VALID|VALIDATED` / Stripe signed webhook) or the admin settle queue — never set PAID manually. Initiation upserts a PROCESSING Payment keyed on `applicationId` or `invoiceId` and records `gateway` + `providerChargeAmount` (minor units). The reconciliation cron never auto-settles. Provider-neutral columns keep their legacy names (`bKashPaymentId`, `gatwayResponse`); `merchantInvoiceNumber` stays the subject key; provider refs are gateway-scoped.
...
- **Owners**: before any property/room/lease write call `getVerifiedOwnerProfile(user.userId)` (requires APPROVED). Scope all queries by `req.user.userId`. Managers scope via `propertyManagerScope`; tenants must be VERIFIED before any payment session (deposit `pay-deposit`, invoice `pay`).
